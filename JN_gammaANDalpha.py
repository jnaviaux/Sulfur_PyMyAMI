...
def CalculateGammaAndAlphas(Tc,S,I,m_cation,m_anion):
    ...
    for cat in range(0,6):
        for an in range(0,7):
            ...
            BMX[cat,an] = beta_0[cat,an] + (beta_1[cat,an]/(2*I)) * (1 - (1+2*sqrtI)*math.exp(-2*sqrtI)) #Eqn 73 of Millero (2007)
            BMX_apostroph[cat,an] = (beta_1[cat,an]/(2*I*I)) * (-1 + (1+(2*sqrtI)+(2*sqrtI))*math.exp(-2*sqrtI)) #Eqn 74 of Millero (2007)
            CMX[cat,an] = C_phi[cat,an]/(2*math.sqrt(-Z_anion[an]*Z_cation[cat])) #Eqn 75 of Millero (2007)
    ...
    for cat in range(0,6):
        for an in range(0,7):
            R = R + m_anion[an]*m_cation[cat]*BMX_apostroph[cat,an] #Eqn 69 of Millero (2007)
            S = S + m_anion[an]*m_cation[cat]*CMX[cat,an] #Eqn 70 of Millero (2007)
...
#print CMX[4,:], "CMX"
    ...
    K_MgOH = math.pow(10,-(3.87-501.6/T))/(gamma_cation[3]*gamma_anion[0]/gamma_MgOH) #JN Note, the fit in math.pow is from Millero (2007) table 12
    K_MgCO3 = math.pow(10,-(1.028+0.0066154*T))/(gamma_cation[3]*gamma_anion[5]/gamma_MgCO3) #JN Note, the fit in math.pow is from Millero (2007) table 12
    K_CaCO3 = math.pow(10,-(1.178+0.0066154*T))/(gamma_cation[4]*gamma_anion[5]/gamma_CaCO3) #JN Note, the fit in math.pow is from Millero (2007) table 12
