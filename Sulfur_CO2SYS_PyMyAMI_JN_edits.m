function [DATA,HEADERS,NICEHEADERS]=CO2SYS(PAR1,PAR2,PAR1TYPE,PAR2TYPE,SAL,TEMPIN,TEMPOUT,PRESIN,PRESOUT,SI,PO4,pHSCALEIN,K1K2CONSTANTS,KSO4CONSTANTS,xB,xST,xCA,xMG);
%**************************************************************************
%
...
%
% Modified by K. Holland
%       added inputs x[B], x[ST], x[Ca], x[Mg]
%       added outputs HSO4 and SO4
%       changed B-Sal relation (as in csysK2.m K.Allen, 2009)
%       changed ST-Sal relation
%       added K* sensitivity to [Ca] and [Mg] from Hain et al. 2015 (choice
%       14) this requires calling the PyMyAMI script (lines 707:714 &
%       1748:1754)
%
%
% Modified by J. Naviaux 1/25/17
%     Mathis PyMyAMI code defines pK_predicted = pK_empirical + delta_pK_MyAMI
%     The matlab script that calls the PyMyAMI script produces a structure K where
%     K.K(1) = modern seawater pK value (unchanging)
%     K.K(2) = Pitzer pK calculation (changes based on Ca, Mg, B etc)
%     K.K(3) = Pitzer pK based on modern seawater (unchanging)
%     
%     The proper way to get the pk values is -log10(K.K(1)) + [-log10(K.K(2) + log10(K.K(3)]
%     previous code only used K.K(2). JN modified to correct calculation.
%     
%     JN also added effect of changing xST (previously did nothing)
...
% 
%  **** SYNTAX EXAMPLES:
%  [Result]                     = CO2SYS(2400,2000,1,2,33,25,25,0,0,1,1,4,14,1,1,1,1,1)
%  [Result,Headers]             = CO2SYS(2400,   8,1,3,35,0,25,4200,0,15,1,1,4,14,1,1,1,1)
%  [Result,Headers,Niceheaders] = CO2SYS( 500,   8,5,3,35,0,25,4200,0,15,1,1,4,14,1,1,1,1)
%  
...
%   xB (frac [boron] w.r.t modern)       : scalar or vector of size n
%   xST (frac [sulfur] w.r.t modern)     : scalar or vector of size n
%   xCA (frac [calcium] w.r.t modern)    : scalar or vector of size n
%   xMG (frac [magnesium] w.r.t modern) : scalar or vector of size n
%
%
...
%  14 = Hain et al, 2015 - see line 666 for T,	S, Ca, Mg, B to produce matrix [K] to call into PyMyAMI     
%           this requires the call PITZER twice, once in the main body and
%           once again at the end for the saturation state calculation 
%           This also requires that the input of Ca, Mg and xB (xB need to
%           occur a bunch of times, lines 628,644,673)
%           Mg and Ca at lines 671 and 672
%           using the PyMyAMI script K0 (665), K1K2(which==14), KSO4 (714)
%           KW (799) KCaKAr (1760) all need to be drawn upon to be updated.
...
%  1 = KSO4 of Dickson & TB of Uppstrom 1979  (ORIGINAL)
%  2 = KSO4 of Khoo    & TB of Uppstrom 1979
%  3 = KSO4 of Dickson & TB of Lee 2010
%  4 = KSO4 of Hain et al, 2015 & TB of Lee 2010
%
%**************************************************************************%
%
...
%    43 - TEMP input           (deg C)     ***    
%    44 - TEMPOUT              (deg C)     ***
%    45 - PRES input           (dbar or m) ***
%    46 - PRESOUT              (dbar or m) ***
%    47 - PAR1TYPE             (integer)   ***
%    48 - PAR2TYPE             (integer)   ***
%    49 - K1K2CONSTANTS        (integer)   ***
%    50 - KSO4CONSTANTS        (integer)   *** 
%    51 - pHSCALE of input     (integer)   ***
%    52 - SAL                  (psu)       ***
%    53 - PO4                  (umol/kgSW) ***
%    54 - SI                   (umol/kgSW) ***
%    55 - K0  input            ()          
...
veclengths=[length(PAR1) length(PAR2) length(PAR1TYPE)...
            ...
            length(xB), length(xST), length(xCA), length(xMG)];
...
Constants(TempCi,Pdbari,xB,xST,xCA,xMG);
...
[OmegaCainp OmegaArinp] = CaSolubility(Sal, TempCi, Pdbari, TCc, PHic,xB,xCA,xMG, xST);
...
Constants(TempCo,Pdbaro,xB,xST,xCA,xMG);
...
[OmegaCaout OmegaArout] = CaSolubility(Sal, TempCo, Pdbaro, TCc, PHoc,xB,xCA,xMG, xST);
...
% Saving data in array, 83 columns, as many rows as samples input
DATA=[TAc*1e6        TCc*1e6         PHic           PCic*1e6        FCic*1e6...
      ...
      OmegaArout     xCO2dryout*1e6  HSO4out*1e3    SO4out*1e3      pHicT...
      pHicS          pHicF           pHicN          pHocT           pHocS...
      pHocF          pHocN           TEMPIN         TEMPOUT         PRESIN...
      PRESOUT        PAR1TYPE        PAR2TYPE       K1K2CONSTANTS   KSO4CONSTANTS...
      pHSCALEIN      SAL             PO4            SI              KIVEC...
      KOVEC          TVEC*1e6];
...
HEADERS={'TAlk';'TCO2';'pHin';'pCO2in';'fCO2in';'HCO3in';'CO3in';...
    ...
    'HSO4out';'SO4out';'pHinTOTAL';'pHinSWS';'pHinFREE';'pHinNBS';...
...
function Constants(TempC,Pdbar,xB,xST,xCA,xMG)
...
%%%%% Added xB here. All return Lee (2010) value for [B]. IGNORE CITATIONS
if any(F)
    xB = 1 ; 
    TB(F) = xB .* 0.0004326.*Sal(F)./35; % in mol/kg-SW
...
if any(F)
	...
	if any(FF)
	    ...
	    xB = 1 ; 
        TB(FF) =  xB .* 0.0004326.*Sal(FF)./35; % in mol/kg-SW
	end
...
TS = xST.*0.02824.*(Sal(F)./35); %(mol/kg) using DOE94
% TS = xST.*(0.14./96.062).*(Sal./1.80655); % in mol/kg-SW

% CalculateK0:
% Weiss, R. F., Marine Chemistry 2:203-215, 1974.
   % Hain et al 2015, Global Biogeochemical Cycles :
    PITZERpath = '/Users/jnaviaux/Desktop/MyAMI-master/JN_Pitzer.py';
    T = TempC ; 
    S = Sal ; 
    Ca = xCA.*0.0102821.* S./35;
    Mg = xMG.*0.0528171.* S./35; 
    B = xB.*S./ 1.80655.* 0.0000219; %JN note: this is close to average of Lee (2010) [B] 0.0004326 and Uppstrom [B] .0004157
    TS = xST.*0.02824*(S./35); %JN added 3-1-19
   [K] = JN_PyMyAMI(PITZERpath,num2str(T),num2str(S),num2str(Ca),num2str(Mg),num2str(TS)); 
    pK0 = -log10(K.K0(1))+(-log10(K.K0(2))+log10(K.K0(3))); 
   K0(F) = 10^-pK0; 
...
if any(F)
   ...
   % Hain et al 2015, Global Biogeochemical Cycles :
    pKS = -log10(K.KSO4(1))+(-log10(K.KSO4(2))+log10(K.KSO4(3))); 
...
if any(F)
    % Hain et al 2015, Global Biogeochemical Cycles :
    pKb = -log10(K.Kb(1))+(-log10(K.Kb(2))+log10(K.Kb(3)));
...
if any(F)
     % Hain et al 2015, Global Biogeochemical Cycles :
    pKw = -log10(K.Kw(1))+(-log10(K.Kw(2))+log10(K.Kw(3))) ; 
...
F=(WhichKs==14);
if any(F)
    % From Hain et al, 2015 for seawater with different [Mg] and [Ca] from modern
	% Global Biogeochemical Cycles
	% The effects of secular calcium and magnesium concnetration changes on
	% the thermodynamics of seawater acid/base chemistry  
   K1(F) = 10^-(-log10(K.K1(1))+(-log10(K.K1(2))+log10(K.K1(3))));  
   K2(F) = 10^-(-log10(K.K2(1))+(-log10(K.K2(2))+log10(K.K2(3))));
   pK1 = -log10(K1) ; 
   pK2 = -log10(K2) ; 
...
function varargout=CaSolubility(Sal, TempC, Pdbar, TC, pH, xB, xCA, xMG, xST)
...
%  in all other cases the rows will be overwritten later on).
   ...
   [K] = JN_PyMyAMI(PITZERpath,num2str(T),num2str(S),num2str(Ca),num2str(Mg),num2str(TS)); 
    ...
    KCa = 10^-(-log10(K.KsC(1))+(-log10(K.KsC(2))+log10(K.KsC(3)))); 
    ...
    KAr = 10^-(-log10(K.KsA(1))+(-log10(K.KsA(2))+log10(K.KsA(3)))) ; 
...
if any(F)
    ...
    % Weiss, R. F., Marine Chemistry 2:203-215, 1974.
   % Hain et al 2015, Global Biogeochemical Cycles : 
    pKCa = -log10(K.KsC(1))+(-log10(K.KsC(2))+log10(K.KsC(3))) ; 
    KCa(F) = 10^-(pKCa);
    ...
    % Weiss, R. F., Marine Chemistry 2:203-215, 1974.
   % Hain et al 2015, Global Biogeochemical Cycles :
    pKAr = -log10(K.KsA(1))+(-log10(K.KsA(2))+log10(K.KsA(3))) ;
