...
import JN_gammaANDalpha
import K_thermo_conditional
import K_HSO4_thermo
import gammaNeutral

# "calculate_gKs" calculates the activity coefficients only (i.e., gamma)
# JN Note 2-15-19, note sure why this "gamma only" section is here. The next section
# does the same calculations, but also multiplies the gammas by the binding constants.
# So maybe this section was for testing stuff earlier?
def calculate_gKs(Tc,Sal, mCa, mMg, xST): #JN added xST
    ...
    m_anion[4] = xST*1e-6*Sal/35 # HSO4 Millero et al., 2008; Dickson OA-guide
    m_anion[5] = 0.0002390*Sal/35 # CO3 Millero et al., 2008; Dickson OA-guide
    m_anion[6] = xST*Sal/35 #JN added, originally 0.0282352*Sal/35 # SO4 Millero et al., 2008; Dickson OA-guide
    
    
    [gamma_cation, gamma_anion, alpha_Hsws, alpha_Ht, alpha_OH, alpha_CO3] = JN_gammaANDalpha.CalculateGammaAndAlphas(Tc,Sal,I,m_cation,m_anion)
...
def calculate_Ks(Tc,Sal, mCa, mMg, xST): #JN added xST
    I = 19.924*Sal/(1000-1.005*Sal)
...
#The first row is for the thermodynamic values for Ks in freshwater
